...
O projeto já inclui um painel pronto em `docs/index.html` — gráficos de
menções por mês, sentimento, tema e campus, mais a lista das notícias
mais recentes, no visual da ACS (azul-marinho + acentos). Ele lê os
dados direto da aba "clipping tratado", sem precisar de servidor nem
backend, hospedado de graça no GitHub Pages.

### Como publicar

1. Na planilha, abra a aba **"clipping tratado"** → `Arquivo` →
   `Compartilhar` → `Publicar na web` → em "Link", escolha a aba
   "clipping tratado" e o formato **CSV** → `Publicar` → copie o link gerado.

   ⚠️ **Isso torna os dados dessa aba acessíveis a qualquer pessoa com o
   link** (não fica listado publicamente em lugar nenhum, mas também não
   é privado). Como o conteúdo é só curadoria de notícias já públicas,
   isso costuma ser um risco baixo — mas é uma escolha consciente, não
   uma obrigação. Se preferirem não publicar, o painel continua
   funcionando com dados de demonstração até vocês decidirem.

2. Abra `docs/index.html`, encontre a linha `const CSV_URL = "COLE_AQUI..."`
   e cole o link copiado no passo 1.

3. No repositório GitHub: `Settings` → `Pages` → em "Source", escolha a
   branch `main` e a pasta `/docs` → `Save`. Depois de 1–2 minutos, o
   painel fica disponível em algo como
   `https://SEU-USUARIO.github.io/clipping-ufsb/`.

Para conferir o visual antes de publicar, basta abrir `docs/index.html`
direto no navegador (duplo clique) — ele mostra dados de demonstração
até o `CSV_URL` real ser configurado.
